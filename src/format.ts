...
export function toMarkdown(report: PostureReport): string {
  const lines: string[] = [];
  lines.push(report.ok ? "# Conditional Access posture ✅" : "# Conditional Access posture ❌");
  lines.push("");
  lines.push(`Generated: \`${report.generatedAt}\``);
  lines.push("");
  lines.push("## Coverage");
  lines.push("");
  lines.push(`- Policy bundles: **${report.bundles}**`);
  lines.push(`- Current bundles: **${report.currentBundles}**`);
  lines.push(`- Gaps: **${report.gaps}**`);
  lines.push(`- Blocking gaps: **${report.blockingGaps}**`);
  lines.push(`- Device gaps: **${report.deviceGaps}**`);
  lines.push(`- Risk gaps: **${report.riskGaps}**`);
...
export function toSummary(report: PostureReport): string {
  ...
  return `${report.bundles} bundles · ${report.gaps} gaps · ${counts.high} high · ${counts.medium} medium (${report.ok ? "ok" : "fail"})`;
}
