...
function layout(title: string, active: string, body: string) {
  const nav = [
    ...
    { href: "/exception-posture", label: "Exception Posture" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Conditional Access Posture Board</div>
        <div class="right">
          <div>synthetic policy exports · gap packets</div>
          <div>entra conditional access · device trust · sign-in risk</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">Microsoft / Entra / Conditional Access proof</span>
            <span class="meta-chip">Synthetic policy snapshots + exception exports</span>
          </div>
          <h1>Conditional Access policy drift, admin exclusions, and device/risk posture that stay operator-readable.</h1>
          <p>This control plane turns Conditional Access snapshots into one identity-governance surface: report-only admin policies, exclusion sprawl, device-trust gaps, missing sign-in risk coverage, weak session controls, and the remediation packets needed before audit or incident windows drift.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Conditional Access, identity protection, and tenant-governance operations for platform, IAM, and security teams.</strong><br />Audit-safe visibility into policy enforcement, exclusions, device trust, and app-targeting posture without exposing tenant credentials.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline posture analyzer + CLI + dashboard surface.</strong><br />This repo includes a reusable analyzer that reads Conditional Access exports and turns them into owner, control-gap, and remediation packets.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Azure / Entra / Conditional Access / Intune / identity governance</strong> should see real admin work: policy enforcement, exclusions, device trust, and audit-safe cleanup proof.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>conditional-access-posture-board · synthetic sample data only</div>
        <div>routes: / · /policy-lane · /control-gaps · /exception-posture · /verification · /docs</div>
      </div>
    </div>
  </body>
</html>`;
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">policy enforcement · exclusions · device trust · risk coverage</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.bundles}</div><div class="lbl">policy bundles</div><div class="h">Synthetic Conditional Access bundles across workforce and privileged-admin scopes.</div></div>
          <div class="kpi green"><div class="v">${metrics.currentBundles}</div><div class="lbl">current bundles</div><div class="h">Snapshots fresh enough to trust for enforcement and audit decisions.</div></div>
          <div class="kpi plum"><div class="v">${metrics.gaps}</div><div class="lbl">gaps</div><div class="h">Observed control deviations across policy, device, risk, session, and app targeting.</div></div>
          <div class="kpi red"><div class="v">${metrics.blockingGaps}</div><div class="lbl">blocking gaps</div><div class="h">Control changes actively weakening the expected Conditional Access posture.</div></div>
          <div class="kpi amber"><div class="v">${metrics.deviceGaps}</div><div class="lbl">device gaps</div><div class="h">Compliant-device enforcement or unmanaged endpoint gates still missing.</div></div>
          <div class="kpi red"><div class="v">${metrics.riskGaps}</div><div class="lbl">risk gaps</div><div class="h">Sign-in risk coverage that needs repair before the policy set is called healthy.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">conditional access proof · recruiter signal</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">guardrails first</div><div class="src-tit">Repair the posture before certifying the tenant</div><p>${metrics.recommendation}</p></div>
          <div class="src"><div class="src-name">control evidence</div><div class="src-tit">Turn policy bundles into operator proof</div><p>Every lane stays tied to owner, control family, resource path, and the next concrete remediation move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show real Conditional Access depth</div><p>This is real Entra Conditional Access proof, not generic cloud copy.</p></div>
        </div>
      </section>`
  );
}
...
export function renderPolicyLane() {
  return layout(
    "Conditional Access Posture Board — Policy Lane",
    "/policy-lane",
    `<section class="section">
...
export function renderControlGaps() {
  return layout(
    "Conditional Access Posture Board — Control Gaps",
    "/control-gaps",
    `<section class="section">
        <div class="sh"><h2>Control Gaps</h2><div class="note">severity · owner · resource</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Risk</th>
...
export function renderExceptionPosture() {
  return layout(
    "Conditional Access Posture Board — Exception Posture",
    "/exception-posture",
    `<section class="section">
        <div class="sh"><h2>Exception Posture</h2><div class="note">packet readiness · blocker · launch window</div></div>
        <div class="board">
          ${exceptionPosture()
            .map(
...
export function renderVerification() {
  return layout(
    "Conditional Access Posture Board — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic sample data, and real Conditional Access posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Conditional Access Posture Board — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/policy-lane</code>, <code>/control-gaps</code>, <code>/exception-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/policy-lane</code>, <code>/api/control-gaps</code>, <code>/api/exception-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline Conditional Access analysis</div><p><code>npx conditional-access-posture fixtures/conditional-access-clean.json --format summary</code> renders the same posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
